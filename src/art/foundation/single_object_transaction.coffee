define [
  ...
  './inspect'
  './clone'
], (Map, FoundationArray, Ruby, BaseObject, Eq, Inspect, Clone) ->

  {removeFirstMatch} = FoundationArray
  {cloneByStructure} = Clone
  {rubyTrue} = Ruby
  {eq} = Eq
  {inspect} = Inspect
  {implementsInterface} = BaseObject

  interpolateableObjectInterface = ["add", "sub", "mul"]

  class SingleObjectTransaction extends BaseObject
